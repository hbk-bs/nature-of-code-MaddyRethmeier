	...
	// you would call background() and loop through stored layer data.
		...
		if (currentLayer < numLayers) {
				for (let k = 0; k < layersPerFrame && currentLayer < numLayers; k++) {
						...
						// Color interpolation - Increased Contrast
						let interColor;
						if (layerIndex < numLayers / 3) {
								 interColor = lerpColor(color(oysterColorsStart[0], oysterColorsStart[1], oysterColorsStart[2]),
																			 ...
																			 map(layerIndex, 0, numLayers / 3, 0, 3)); // Further exaggerate interpolation
						}
						else {
								 interColor = lerpColor(color(oysterColorsMid[0], oysterColorsMid[1], oysterColorsMid[2]),
																			 ...
																			 map(layerIndex, numLayers / 3, numLayers, -2, 1)); // Further exaggerate interpolation
						}
						
						let thickness = random(1, 2.5);
						strokeWeight(thickness);
						stroke(hue(interColor), saturation(interColor), brightness(interColor), 0.9); // Increased saturation
			...
			if (pauseCouner > 100) {
				pauseCouner = 0;
				currentLayer = 0; // Reset layer count for the next drawing
				background(255); // Clear the canvas for the next drawing
			}
